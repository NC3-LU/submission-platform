            ...
            <div class="bg-white dark:bg-gray-800 shadow rounded-xl overflow-hidden">
                @if($forms->isEmpty())
                    <div class="text-center py-12">
                        ...
                        <a href="{{ route('forms.create') }}" class="mt-3 inline-flex items-center text-sm text-sky-700 dark:text-sky-400 hover:text-sky-700 dark:hover:text-sky-300 font-medium transition-colors">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                        <div class="flex items-center gap-3">
                                            @if($form->user_id === Auth::id() || $form->appointedUsers()->where('user_id', Auth::id())->where('can_edit', true)->exists())
                                                <a href="{{ route('forms.edit', $form) }}" class="text-sky-700 dark:text-sky-400 hover:text-sky-700 dark:hover:text-sky-300 transition-colors">Edit</a>
                                            @endif
                                            <a href="{{ route('submissions.index', $form) }}" class="text-sky-700 dark:text-sky-400 hover:text-sky-700 dark:hover:text-sky-300 transition-colors">Submissions</a>
                                            @if($form->status === 'published')
                                                <a href="{{ route('forms.preview', $form) }}" class="text-sky-700 dark:text-sky-400 hover:text-sky-700 dark:hover:text-sky-300 transition-colors">Preview</a>
                                            @endif
                                            @can('duplicate', $form)
                                                <form action="{{ route('forms.duplicate', $form) }}" method="POST" class="inline">
                                                    @csrf
                                                    <button type="submit" class="text-sky-700 dark:text-sky-400 hover:text-sky-700 dark:hover:text-sky-300 transition-colors text-sm font-medium">
                                                        Clone
                                                    </button>
                                                </form>
                                            @endcan
