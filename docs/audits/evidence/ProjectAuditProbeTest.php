...
class ProjectAuditProbeTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        if (getenv('APP_ENV') !== 'testing' || getenv('DB_CONNECTION') !== 'sqlite' || getenv('DB_DATABASE') !== ':memory:') {
            throw new \RuntimeException('Audit probes require explicit APP_ENV=testing DB_CONNECTION=sqlite DB_DATABASE=:memory: before any application bootstrap.');
        }
        parent::setUp();
    }
    ...
    public function test_forged_temp_path_must_not_move_another_submissions_file(): void
    {
        ...
        Livewire::test(SubmissionForm::class, ['form' => $form])->set("fieldValues.{$field->id}", 'temp-submissions/../'.$path)->call('submit');
    ...
    public function test_repeat_autosave_must_preserve_file_reference(): void
    {
        ...
        [$owner,$form,$field] = $this->fixture('file');
        $user = User::factory()->create();
        $component = Livewire::actingAs($user)->test(SubmissionForm::class, ['form' => $form])->set("tempFiles.field_{$field->id}", UploadedFile::fake()->create('audit.pdf', 10, 'application/pdf'))->call('saveAsDraft');
    ...
    public function test_required_checkbox_must_have_selection(): void
    {
        ...
        Livewire::test(SubmissionForm::class, ['form' => $form])->set("fieldValues.{$field->id}", [false, false])->call('submit');
        $this->assertDatabaseMissing('submissions', ['form_id' => $form->id, 'status' => 'submitted']);
    }
    ...
    public function test_rate_limit_response_must_be_429(): void
    {
        ...
        $response = null;
        for ($i = 0; $i < 65; $i++) {
            $response = $this->withToken('synthetic-audit-token')->getJson('/api/v1/forms');
