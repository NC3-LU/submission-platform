...
$fileField = $cat2->fields()->create(['form_id' => $form->id, 'type' => 'file', 'label' => 'Attachment', 'required' => false, 'order' => 3]);
...
$scanned = Submission::factory()->submitted()->create(['form_id' => $form->id, 'user_id' => $user->id]);
$path = 'submissions/'.$scanned->id.'/scan-fixture.pdf';
Storage::disk('private')->put($path, '%PDF-1.4 synthetic fixture');
$file = $scanned->values()->create(['form_field_id' => $fileField->id, 'value' => $path]);
