...
php -r '$m=json_decode(file_get_contents($argv[1]."/build/manifest.json"), true, 512, JSON_THROW_ON_ERROR); foreach($m as $e) { foreach(array_merge([$e["file"]],$e["css"]??[]) as $f) { if (!is_file($argv[1]."/build/".$f)) exit(1); }}' "$asset_tmp"
